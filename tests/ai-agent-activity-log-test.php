...
aial_test_eq( AI_Agent_Activity_Log_Channel::current(), '', 'a browser request is not a channel' );

$GLOBALS['aial_stub_rest_request'] = true;
aial_test_eq( AI_Agent_Activity_Log_Channel::current(), 'rest', 'a REST request is' );
...
aial_test_eq( AI_Agent_Activity_Log_Channel::current(), '', 'a cookie-authenticated REST request - a person saving in the block editor - is not an automation' );
...
aial_test_eq( AI_Agent_Activity_Log_Channel::current(), 'rest', 'while a REST write authenticated by an application password is still rest, cookie or no cookie' );
$GLOBALS['aial_stub_app_password_uuid'] = null;
$GLOBALS['aial_stub_app_passwords']     = array();
...
aial_test_eq( AI_Agent_Activity_Log_Channel::current(), 'rest', 'a cookie that was sent and rejected is no browser session, so the request is still rest' );
...
aial_test_eq( AI_Agent_Activity_Log_Channel::current(), 'cron', 'a cron run is decided before the cookie is ever looked at' );
$GLOBALS['aial_stub_doing_cron'] = false;
...
aial_test_eq( AI_Agent_Activity_Log_Channel::current(), 'rest', 'and an unauthenticated REST write, which no browser session backs, stays on the recorded side' );

// Contexts nest, and the outermost one is the true origin. Cron that runs
// code setting REST_REQUEST is still cron.
$GLOBALS['aial_stub_doing_cron'] = true;
aial_test_eq( AI_Agent_Activity_Log_Channel::current(), 'cron', 'cron outranks REST when both are true' );
...
aial_test_ok( AI_Agent_Activity_Log_Channel::is_read_request(), 'GET is a read' );
$_SERVER['REQUEST_METHOD'] = 'HEAD';
aial_test_ok( AI_Agent_Activity_Log_Channel::is_read_request(), 'so is HEAD' );
$_SERVER['REQUEST_METHOD'] = 'POST';
aial_test_ok( ! AI_Agent_Activity_Log_Channel::is_read_request(), 'POST is not' );
$_SERVER['REQUEST_METHOD'] = 'DELETE';
aial_test_ok( ! AI_Agent_Activity_Log_Channel::is_read_request(), 'nor is DELETE' );
...
aial_test_ok( ! AI_Agent_Activity_Log_Channel::is_read_request(), 'and a request with no method at all is not a read' );
...
$_SERVER['REQUEST_METHOD']      = 'GET';
$GLOBALS['aial_stub_doing_cron'] = true;
aial_test_ok( AI_Agent_Activity_Log_Channel::is_early_channel(), 'a GET-triggered cron run is a channel we can already name' );

$GLOBALS['aial_stub_doing_cron'] = false;
aial_test_ok( ! AI_Agent_Activity_Log_Channel::is_early_channel(), 'a plain browser request is not' );
...
// channel at all".
$GLOBALS['aial_stub_rest_request'] = true;
aial_test_ok( ! AI_Agent_Activity_Log_Channel::is_early_channel(), 'and REST is not, because REST_REQUEST does not exist yet at plugins_loaded' );
$GLOBALS['aial_stub_rest_request'] = false;
unset( $_SERVER['REQUEST_METHOD'] );
...
aial_test_eq( AI_Agent_Activity_Log_Channel::app_name(), 'ContentEngine', 'a resolved application password returns its name, not the User-Agent' );
...
$GLOBALS['aial_stub_app_passwords']     = array();
aial_test_eq( AI_Agent_Activity_Log_Channel::app_name(), '', 'a UUID with no matching record has no app name' );

// No application password authenticated this request at all.
$GLOBALS['aial_stub_app_password_uuid'] = null;
aial_test_eq( AI_Agent_Activity_Log_Channel::app_name(), '', 'an unidentified caller has no app name, and the User-Agent is not one' );
...
AI_Agent_Activity_Log_Store::set_writer( $writer );

aial_test_eq( AI_Agent_Activity_Log_Store::table(), 'wp_ai_agent_activity_log', 'the table is named from the writer prefix' );

AI_Agent_Activity_Log_Store::insert(
	array(
...
aial_test_eq( $writer->inserted[0]['table'], 'wp_ai_agent_activity_log', 'against the right table' );
...
AI_Agent_Activity_Log_Store::insert( array() );
...
$writer->inserted = array();
AI_Agent_Activity_Log_Store::insert( array( 'object_name' => 'About us', 'object_subtype' => 'page' ) );
...
$writer->inserted = array();
AI_Agent_Activity_Log_Store::insert( array( 'object_name' => str_repeat( 'ת', 300 ) ) );
...
$writer->inserted = array();
AI_Agent_Activity_Log_Store::insert( array( 'object_name' => 'שלום עולם' ) );
...
AI_Agent_Activity_Log_Store::insert( array( 'fields' => $many ) );
$encoded = $writer->inserted[0]['data']['fields'];
aial_test_ok( strlen( wp_json_encode( $many ) ) > AI_Agent_Activity_Log_Store::MAX_FIELDS_BYTES, 'the unbounded encoding of 2000 long meta keys really does exceed the budget' );
aial_test_ok( strlen( $encoded ) <= AI_Agent_Activity_Log_Store::MAX_FIELDS_BYTES, 'what is written is inside the budget' );
...
$writer->inserted = array();
AI_Agent_Activity_Log_Store::insert( array( 'fields' => array( 'post_content', 'rank_math_title' ) ) );
...
AI_Agent_Activity_Log_Store::insert( array( 'fields' => $hebrew ) );
$encoded = $writer->inserted[0]['data']['fields'];
$decoded = json_decode( $encoded, true );
aial_test_ok( strlen( $encoded ) <= AI_Agent_Activity_Log_Store::MAX_FIELDS_BYTES, 'a list of non-ASCII names is bounded by its encoded bytes' );
...
aial_test_ok( strlen( $encoded ) > AI_Agent_Activity_Log_Store::MAX_FIELDS_BYTES - 200, 'the bytes written fill the budget' );
...
aial_test_ok( $kept_chars < AI_Agent_Activity_Log_Store::MAX_FIELDS_BYTES - 10000, 'while the character count of the names actually kept is far short of the byte budget, which is the room a character-counting bound would have kept filling' );

/* ---- query arguments ---- */

$q = AI_Agent_Activity_Log_Store::query_args( array( 'channel' => 'rest', 'object_type' => 'post', 'object_id' => 812, 'per_page' => 20, 'page' => 2 ) );
...
$q = AI_Agent_Activity_Log_Store::query_args( array( 'channel' => 'ftp; DROP TABLE wp_posts' ) );
aial_test_eq( $q['params'], array(), 'a channel outside the enum contributes no parameter' );
aial_test_eq( $q['where'], '', 'and no clause' );

$q = AI_Agent_Activity_Log_Store::query_args( array( 'per_page' => 5000 ) );
aial_test_eq( $q['limit'], 100, 'per_page is capped at 100' );
$q = AI_Agent_Activity_Log_Store::query_args( array( 'per_page' => 0, 'page' => 0 ) );
...
$plan = AI_Agent_Activity_Log_Store::prune_plan( 30, 20000, $now );
aial_test_eq( count( $plan ), 2, 'both bounds produce work when both are set' );

$plan = AI_Agent_Activity_Log_Store::prune_plan( 0, 20000, $now );
...
$plan = AI_Agent_Activity_Log_Store::prune_plan( 30, -5, $now );
...
aial_test_eq( AI_Agent_Activity_Log_Store::prune_plan( 0, 0, $now ), array(), 'and both disabled means no work at all, rather than a delete with no bound' );

$plan = AI_Agent_Activity_Log_Store::prune_plan( 30, 0, $now );
...
AI_Agent_Activity_Log_Buffer::record( 'post', 'page', 812, 'updated', 'About us', array( 'post_content' ) );
AI_Agent_Activity_Log_Buffer::record( 'post', 'page', 812, 'updated', 'About us', array( 'rank_math_title' ) );
AI_Agent_Activity_Log_Buffer::record( 'post', 'page', 812, 'updated', 'About us', array( '_elementor_data', 'post_content' ) );

$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', 'ContentEngine', 5, 1756108800 );
...
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Buffer::record( 'post', 'page', 812, 'updated', 'About us', array( 'post_title' ) );
AI_Agent_Activity_Log_Buffer::record( 'term', 'category', 4, 'updated', 'News', array( 'name' ) );
AI_Agent_Activity_Log_Buffer::record( 'post', 'page', 812, 'updated', 'About us', array( 'post_excerpt' ) );
aial_test_eq( count( AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 ) ), 2, 'two objects are two rows' );
...
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Buffer::record( 'post', 'page', 812, 'updated', 'About us', array( 'post_title' ) );
AI_Agent_Activity_Log_Buffer::record( 'post', 'page', 812, 'deleted', 'About us' );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Buffer::record( 'post', 'page', 813, 'created', 'New page' );
AI_Agent_Activity_Log_Buffer::record( 'post', 'page', 813, 'updated', 'New page', array( 'post_content' ) );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
AI_Agent_Activity_Log_Buffer::reset();
aial_test_eq( AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 ), array(), 'a request that changed nothing writes nothing' );
...
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Buffer::record( 'plugin', 'akismet/akismet.php', 0, 'activated', 'Akismet' );
AI_Agent_Activity_Log_Buffer::record( 'plugin', 'hello-dolly/hello.php', 0, 'activated', 'Hello Dolly' );
$rows  = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Buffer::record( 'option', '', 0, 'updated', 'siteurl' );
AI_Agent_Activity_Log_Buffer::record( 'option', '', 0, 'updated', 'blogname' );
$rows  = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
$names = array_column( $rows, 'object_name' );
...
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Buffer::record( 'plugin', 'akismet/akismet.php', 0, 'activated', 'Akismet' );
AI_Agent_Activity_Log_Buffer::record( 'plugin', 'akismet/akismet.php', 0, 'updated', 'Akismet' );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
aial_test_eq( $rows[0]['action'], 'activated', 'activated recorded before updated on the same key keeps activated' );

AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Buffer::record( 'theme', '', 0, 'switched', 'Twenty Twenty-Five' );
AI_Agent_Activity_Log_Buffer::record( 'theme', '', 0, 'updated', 'Twenty Twenty-Five' );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
aial_test_eq( $rows[0]['action'], 'switched', 'switched is not overwritten by a later updated' );

AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Buffer::record( 'plugin', 'akismet/akismet.php', 0, 'deactivated', 'Akismet' );
AI_Agent_Activity_Log_Buffer::record( 'plugin', 'akismet/akismet.php', 0, 'updated', 'Akismet' );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Buffer::record( 'theme', '', 0, 'switched', 'Twenty Twenty-Four' );
AI_Agent_Activity_Log_Buffer::record( 'theme', '', 0, 'switched', 'Twenty Twenty-Five' );
$rows  = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
$names = array_column( $rows, 'object_name' );
...
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Buffer::record( 'theme', '', 0, 'activated', 'Twenty Twenty-Five' );
AI_Agent_Activity_Log_Buffer::record( 'theme', '', 0, 'switched', 'Twenty Twenty-Five' );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Buffer::record( 'post', 'page', 814, 'created', 'Draft' );
AI_Agent_Activity_Log_Buffer::record( 'post', 'page', 814, 'deleted', 'Draft' );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
// later still loses.
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Buffer::record( 'plugin', 'akismet/akismet.php', 0, 'deleted', 'Akismet' );
AI_Agent_Activity_Log_Buffer::record( 'plugin', 'akismet/akismet.php', 0, 'deactivated', 'Akismet' );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
aial_test_eq( AI_Agent_Activity_Log_Buffer::max_age_days(), 30, 'the age bound defaults to thirty days' );
aial_test_eq( AI_Agent_Activity_Log_Buffer::max_rows(), 20000, 'and the row bound to twenty thousand' );

add_filter( 'ai_agent_activity_log_max_age_days', function () { return 7; } );
aial_test_eq( AI_Agent_Activity_Log_Buffer::max_age_days(), 7, 'both are filterable' );
add_filter( 'ai_agent_activity_log_max_rows', function () { return 0; } );
aial_test_eq( AI_Agent_Activity_Log_Buffer::max_rows(), 0, 'including down to zero, which disables that bound' );
$GLOBALS['aial_stub_filters'] = array();
...
$diff = AI_Agent_Activity_Log_Hooks::post_field_diff( $after, $before );
...
aial_test_eq( AI_Agent_Activity_Log_Hooks::post_field_diff( $after, null ), array(), 'a create reports no field diff at all' );

/* ---- the option allowlist ---- */

$GLOBALS['aial_stub_filters'] = array();
$watched = AI_Agent_Activity_Log_Hooks::watched_options();
...
add_filter( 'ai_agent_activity_log_watched_options', function ( $list ) { $list[] = 'my_option'; return $list; } );
aial_test_ok( in_array( 'my_option', AI_Agent_Activity_Log_Hooks::watched_options(), true ), 'and a site may add one by filter' );
// A filter returning something that is not a list must not disarm the
// allowlist into "watch everything".
add_filter( 'ai_agent_activity_log_watched_options', function () { return 'nonsense'; } );
aial_test_ok( in_array( 'siteurl', AI_Agent_Activity_Log_Hooks::watched_options(), true ), 'while a filter returning nonsense leaves the defaults standing' );
$GLOBALS['aial_stub_filters'] = array();
...
AI_Agent_Activity_Log_Store::set_writer( $writer );
$writer->var = 900;
AI_Agent_Activity_Log_Store::prune( 30, 20000, 1756108800 );
...
$writer->var     = null;
AI_Agent_Activity_Log_Store::prune( 0, 20000, 1756108800 );
aial_test_eq( count( $writer->queries ), 1, 'a table under the row cap issues the lookup and no delete' );

$writer->queries = array();
AI_Agent_Activity_Log_Store::prune( 0, 0, 1756108800 );
...
AI_Agent_Activity_Log_Hooks::on_post_saved( 601, $post_after, true, $post_before );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
AI_Agent_Activity_Log_Hooks::on_post_saved( 602, $after, true, $before );
AI_Agent_Activity_Log_Hooks::on_post_meta( 101, 602, 'rank_math_title' );
AI_Agent_Activity_Log_Hooks::on_post_meta( 102, 602, '_elementor_data' );
AI_Agent_Activity_Log_Hooks::on_post_meta( 103, 602, 'custom_field' );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
AI_Agent_Activity_Log_Hooks::on_post_saved( 603, (object) array( 'post_type' => 'revision' ), false, null );
aial_test_eq( AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 ), array(), 'a revision records nothing' );

AI_Agent_Activity_Log_Buffer::reset();
$GLOBALS['aial_stub_autosaves'] = array( 604 );
AI_Agent_Activity_Log_Hooks::on_post_saved( 604, (object) array( 'post_type' => 'post' ), false, null );
aial_test_eq( AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 ), array(), 'and an autosave records nothing either' );
...
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Hooks::on_post_deleted( 605, (object) array( 'post_type' => 'post', 'post_title' => 'Gone' ) );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Hooks::on_post_deleted( 606, (object) array( 'post_type' => 'attachment', 'post_title' => 'Image' ) );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
AI_Agent_Activity_Log_Hooks::on_post_deleted( 608, (object) array( 'post_type' => 'revision' ) );
aial_test_eq( AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 ), array(), 'deleting a revision records nothing' );

AI_Agent_Activity_Log_Buffer::reset();
$GLOBALS['aial_stub_autosaves'] = array( 609 );
AI_Agent_Activity_Log_Hooks::on_post_deleted( 609, (object) array( 'post_type' => 'post' ) );
aial_test_eq( AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 ), array(), 'deleting an autosave records nothing' );
$GLOBALS['aial_stub_autosaves'] = array();
...
AI_Agent_Activity_Log_Hooks::on_post_meta( 201, 610, 'rank_math_title' );
aial_test_eq( AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 ), array(), 'added_post_meta on a revision id records nothing' );

AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Hooks::on_post_meta( 202, 610, 'rank_math_title' );
aial_test_eq( AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 ), array(), 'deleted_post_meta on a revision id records nothing either' );
...
AI_Agent_Activity_Log_Hooks::on_post_meta( 203, 611, 'rank_math_title' );
aial_test_eq( AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 ), array(), 'and an autosave id records nothing through the meta callback' );
$GLOBALS['aial_stub_autosaves'] = array();
...
AI_Agent_Activity_Log_Hooks::on_post_meta( 204, 612, 'rank_math_title' );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
AI_Agent_Activity_Log_Hooks::on_post_saved( 607, $attachment_after, false, null );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
AI_Agent_Activity_Log_Hooks::on_term_edited( 30, 300, 'category' );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
AI_Agent_Activity_Log_Hooks::on_term_deleted( 31, 301, 'category', $deleted_term );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
AI_Agent_Activity_Log_Hooks::on_user_role( 70, 'editor' );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Hooks::on_user_created( 71 );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
aial_test_eq( $rows[0]['action'], 'created', 'on_user_created() records created' );

AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Hooks::on_user_updated( 71 );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
aial_test_eq( $rows[0]['action'], 'updated', 'on_user_updated() records updated' );

AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Hooks::on_user_deleted( 71 );
$rows = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
...
// silently a task ago.
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Hooks::on_plugin_activated( 'akismet/akismet.php' );
AI_Agent_Activity_Log_Hooks::on_plugin_activated( 'hello-dolly/hello.php' );
$rows  = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
$names = array_column( $rows, 'object_name' );
...
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Hooks::on_option_updated( 'siteurl' );
AI_Agent_Activity_Log_Hooks::on_option_updated( 'blogname' );
$rows  = AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 );
$names = array_column( $rows, 'object_name' );
...
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Hooks::on_option_updated( '_transient_doing_cron' );
aial_test_eq( AI_Agent_Activity_Log_Buffer::rows( 'rest', '', 5, 1756108800 ), array(), 'an option outside the allowlist records nothing' );
...
function aial_stub_recording( $on = true ) {
	if ( $on ) {
		update_option( 'ai_agent_activity_log_schema', AI_Agent_Activity_Log_Store::SCHEMA_VERSION );
	} else {
		update_option( 'ai_agent_activity_log_schema', '' );
	}
}
...
AI_Agent_Activity_Log_Buffer::reset();
AI_Agent_Activity_Log_Hooks::on_option_updated( 'siteurl' );
AI_Agent_Activity_Log_Hooks::flush();
aial_test_eq( $writer->inserted, array(), 'flush() on a browser request writes no row' );
aial_test_eq( AI_Agent_Activity_Log_Buffer::pending(), array(), 'and empties the buffer rather than leaving it for the next request' );
...
aial_test_ok( ! isset( $GLOBALS['aial_stub_options']['ai_agent_activity_log_last_prune'] ), 'and does not stamp the prune throttle' );
...
aial_test_eq( AI_Agent_Activity_Log_Buffer::pending(), array(), 'and the buffer is empty afterwards' );
aial_test_ok( ! empty( $writer->queries ), 'a first flush prunes, since the throttle has never been stamped' );
aial_test_ok( isset( $GLOBALS['aial_stub_options']['ai_agent_activity_log_last_prune'] ), 'stamping the throttle as it goes' );
...
$GLOBALS['aial_stub_options']['ai_agent_activity_log_last_prune'] = time() - HOUR_IN_SECONDS - 1;
$writer->inserted = array();
$writer->queries  = array();
AI_Agent_Activity_Log_Hooks::on_option_updated( 'blogname' );
AI_Agent_Activity_Log_Hooks::flush();
...
// invisible to AI_Agent_Activity_Log_Store::table().
$writer = new AIAL_Test_Writer();
AI_Agent_Activity_Log_Store::set_writer( $writer );
$GLOBALS['wpdb'] = $writer;
...
AI_Agent_Activity_Log_Buffer::record( 'post', 'post', 5, 'updated', 'Home on site two', array( 'post_title' ) );
switch_to_blog( 7 );
aial_stub_recording();
AI_Agent_Activity_Log_Buffer::record( 'post', 'post', 5, 'updated', 'Home on site seven', array( 'post_content' ) );
...
aial_test_eq(
	...
	array( 'wp_2_ai_agent_activity_log', 'wp_7_ai_agent_activity_log' ),
...
aial_test_ok(
	(bool) array_filter( $writer->queries, function ( $sql ) { return false !== strpos( $sql, 'wp_7_ai_agent_activity_log' ); } ),
	'the prune trims the switched-to site table too, rather than trimming the originating site twice'
);
aial_test_ok(
	(bool) array_filter( $writer->queries, function ( $sql ) { return false !== strpos( $sql, 'wp_2_ai_agent_activity_log' ); } ),
	'and the originating site table as well'
);
aial_test_ok( isset( $GLOBALS['aial_stub_options']['ai_agent_activity_log_last_prune'] ), 'the throttle stamp lands on the originating site' );
aial_test_ok( isset( $GLOBALS['aial_stub_blog_options'][7]['ai_agent_activity_log_last_prune'] ), 'and a separate one on the site that was switched to, so neither starves the other' );
...
aial_stub_enter_blog( 2 );
aial_stub_recording();
AI_Agent_Activity_Log_Buffer::record( 'post', 'post', 5, 'updated', 'Home on site two' );
switch_to_blog( 7 );
aial_stub_recording( false );
AI_Agent_Activity_Log_Buffer::record( 'post', 'post', 5, 'updated', 'Home on site seven' );
restore_current_blog();
$writer->inserted = array();
$writer->queries  = array();
AI_Agent_Activity_Log_Hooks::flush();
...
aial_test_eq(
	array_column( $writer->inserted, 'table' ),
	array( 'wp_2_ai_agent_activity_log' ),
	'a site with the module switched off receives no rows, while the site that has it on still does'
);
aial_test_ok(
	! array_filter( $writer->queries, function ( $sql ) { return false !== strpos( $sql, 'wp_7_ai_agent_activity_log' ); } ),
...
$writer->inserted = array();
$writer->queries  = array();
AI_Agent_Activity_Log_Buffer::record( 'post', 'post', 5, 'updated', 'Home', array( 'post_title' ) );
AI_Agent_Activity_Log_Buffer::record( 'post', 'post', 9, 'updated', 'About', array( 'post_title' ) );
AI_Agent_Activity_Log_Hooks::flush();

aial_test_eq(
	array_column( $writer->inserted, 'table' ),
	array( 'wp_ai_agent_activity_log', 'wp_ai_agent_activity_log' ),
...
$thrower->fail_table = 'wp_7_ai_agent_activity_log';
AI_Agent_Activity_Log_Store::set_writer( $thrower );
...
AI_Agent_Activity_Log_Hooks::init();
aial_test_eq(
	aial_stub_filter_priority( 'shutdown', array( 'AI_Agent_Activity_Log_Hooks', 'flush' ) ),
	9999,
...
require_once dirname( __DIR__ ) . '/includes/class-ai-agent-activity-log-rest.php';

/* ---- the endpoint's argument schema ---- */

$args = AI_Agent_Activity_Log_Rest::args();
...
aial_test_ok( ! AI_Agent_Activity_Log_Rest::may_read(), 'a user without manage_options may not read the log' );
$GLOBALS['aial_stub_denied_caps'] = array();
aial_test_ok( AI_Agent_Activity_Log_Rest::may_read(), 'and one with it may' );

/* ---- there is no way to erase it over the API ---- */

$GLOBALS['aial_stub_rest_routes'] = array();
AI_Agent_Activity_Log_Rest::init();
$key = 'ai-agent-activity-log/v1/activity';
aial_test_ok( isset( $GLOBALS['aial_stub_rest_routes'][ $key ] ), 'the route is registered under ai-agent-activity-log/v1/activity' );
$methods = array();
...
$response = AI_Agent_Activity_Log_Rest::handle( new WP_REST_Request( array() ) );
...
$writer->queries = array();
AI_Agent_Activity_Log_Rest::handle( new WP_REST_Request( array() ) );
...
$writer->queries = array();
AI_Agent_Activity_Log_Rest::handle( new WP_REST_Request( array( 'channel' => 'cron' ) ) );
...
$GLOBALS['aial_stub_options'] = array( 'ai_agent_activity_log_schema' => AI_Agent_Activity_Log_Store::SCHEMA_VERSION );
...
$GLOBALS['aial_stub_dbdelta_calls'] = 0;
$writer->queries                   = array();
$writer->var                       = 'wp_ai_agent_activity_log';
AI_Agent_Activity_Log_Store::install_table();
aial_test_eq( $GLOBALS['aial_stub_dbdelta_calls'], 1, 'an unstamped schema version runs dbDelta' );
aial_test_eq( get_option( 'ai_agent_activity_log_schema', 'unstamped' ), AI_Agent_Activity_Log_Store::SCHEMA_VERSION, 'and stamps the version once the table is confirmed present' );

// _ is a single-character wildcard in LIKE and every table prefix has one, so
// an unescaped name would match wpXaiXagentXactivityXlog and report a table
// this plugin never created as its own.
aial_test_ok(
	in_array( "SHOW TABLES LIKE 'wp\_ai\_agent\_activity\_log'", $writer->queries, true ),
...
aial_test_eq( get_option( 'ai_agent_activity_log_schema', 'unstamped' ), 'unstamped', 'but leaves the version unstamped, because a stamp would claim a table that is not there' );
...
$writer->var = 'wp_ai_agent_activity_log';
AI_Agent_Activity_Log_Store::install_table();
aial_test_eq( get_option( 'ai_agent_activity_log_schema', 'unstamped' ), AI_Agent_Activity_Log_Store::SCHEMA_VERSION, 'and stamps the version on the retry that finally works' );

/* ---- the screen ---- */

require_once dirname( __DIR__ ) . '/includes/class-ai-agent-activity-log-admin.php';
...
function aial_render( $get ) {
	...
	$writer->queries = array();
	ob_start();
	$admin = new AI_Agent_Activity_Log_Admin();
	$admin->render_page();
...
// it gave AI_Agent_Activity_Log_Store::set_writer(), exactly as production has one $wpdb
// serving both roles.
//
...
define( 'WP_UNINSTALL_PLUGIN', 'ai-agent-activity-log/ai-agent-activity-log.php' );
...
aial_test_eq( $wpdb->queries, array( 'DROP TABLE IF EXISTS `wp_ai_agent_activity_log`' ), 'a single site drops its one log table' );
aial_test_eq( $GLOBALS['aial_stub_switch_calls'], 0, 'and switches no blogs, because there are none to switch to' );
$aial_single_deleted = array_values( array_filter( $GLOBALS['aial_stub_deleted'], function ( $entry ) { return false !== strpos( $entry, 'ai_agent_activity_log' ); } ) );
aial_test_eq( $aial_single_deleted, array( '1:ai_agent_activity_log_schema', '1:ai_agent_activity_log_last_prune' ), 'and deletes both of the log stamps' );
...
aial_test_eq(
	$wpdb->queries,
	array(
		'DROP TABLE IF EXISTS `wp_ai_agent_activity_log`',
		'DROP TABLE IF EXISTS `wp_2_ai_agent_activity_log`',
		'DROP TABLE IF EXISTS `wp_4_ai_agent_activity_log`',
	),
	'every site on the network loses its own log table, named from that site prefix'
);

$aial_network_deleted = array_values( array_filter( $GLOBALS['aial_stub_deleted'], function ( $entry ) { return false !== strpos( $entry, 'ai_agent_activity_log_' ); } ) );
aial_test_eq(
	$aial_network_deleted,
	array(
		'1:ai_agent_activity_log_schema',
		'1:ai_agent_activity_log_last_prune',
		'2:ai_agent_activity_log_schema',
		'2:ai_agent_activity_log_last_prune',
		'4:ai_agent_activity_log_schema',
		'4:ai_agent_activity_log_last_prune',
	),
...
aial_test_eq(
	...
	array( 'ai_agent_activity_log_schema', 'ai_agent_activity_log_last_prune' ),
